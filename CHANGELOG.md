...
小版本界面修正，保持 v0.4.0 的功能和本机数据边界不变。

### 调整

- 悬浮球收敛为 54×54 双环：中心使用 5h 剩余额度数字；未知状态明确显示 `--`。
- 主菜单直接展示 5h、7d 额度和完整重置日期时间；刷新、Usage statistics、Settings、Quit 保持一级入口。
- Usage statistics 保留周期、Token 总量、Input/Cached/Output、趋势和快捷键；Settings 保留悬浮球、Open Codex、Open at Login、Clear Local Data...。
- 自绘额度行保留 VoiceOver 文本；可操作菜单项使用原生 `NSMenuItem`，避免快捷键和长趋势文案被自绘布局吞掉。
- 悬浮详情改为固定列和截断保护，最长合法重置时间也不会溢出气泡边界。

### 验证边界

- 原生 `ui_snapshot_driver.swift` 覆盖 0、9、59、100、无数据五种悬浮球状态及有效/错误详情文本。
- 正式代码不使用结束 tracking 后自动重开菜单。物理鼠标点击、真实子菜单展开、多屏和全屏边界不在本轮自动证据中，仍需正式 UI 复核。
