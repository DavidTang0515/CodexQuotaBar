...
- 保留 54×54 石墨双环；提高菜单栏字号，数字右对齐，以小色标表达额度状态。
- 菜单栏条形保留原来的浅橙、浅绿及浅蓝空格配色；构建号为 8。
- 主菜单和悬停详情统一数字层级；中文和英文随系统语言切换。
- Token 用量采用两列排版，缓存作为输入子项；五个统计周期可从原生子菜单直接选择，保留 ⌘T 轮换。
- 失败时保留旧值并显示更新时间；修复失败后切换外观的额度丢失，限制自动快速重试次数。
- 悬停延迟、轻量淡入淡出、详情固定、Esc 关闭及可访问性按钮支持。
- 构建支持 `CQB_BUILD_ROOT`，候选可独立构建，保留原应用。

验证：22 项 Python 回归测试、中英文 Swift 状态和菜单选择测试、原生控件亮暗预览、构建与签名通过。实机已验证辅助技术点击小球展开及 Esc 收起；完整菜单指针操作、拖动与跨屏仍待进一步验证。

要求：Apple Silicon、macOS 13 或更新版本。额度读取依赖本地 Codex app-server 的 ChatGPT 账号认证；未认证时会显示未知额度。应用使用 ad-hoc 签名，未公证。
